        ...
        if($info=C('cms:form:get',@$_POST['id'])){
            ...
            if($info_edit_array['inputhash']==$info['inputhash']) {
                ...
                foreach($info_config as $val) {
                    ...
                    $val['auth']['all']=true;
                    $info_edit_array['configs'][$val['name']]=C('cms:input:post',$val);
                    if($info_edit_array['configs'][$val['name']]===false) {
                        Return E('配置项:'.$val['configname'].' 不正确');
                    }elseif(is_array($info_edit_array['configs'][$val['name']])) {
                        if(isset($info_edit_array['configs'][$val['name']]['error'])) {
                            Return E('配置项:'.$val['configname'].' '.$info_edit_array['configs'][$val['name']]['error']);
                        }else {
                            Return E('配置项:'.$val['configname'].' 不正确');
                        }
                    }
            ...
            if(C('cms:form:edit',$info_edit_array)) {
                if(isset($_POST['classhash']) && $_POST['classhash']!=$info['classhash']) {
                    if(!is_hash($_POST['classhash'])){
                        $_POST['classhash']='admin';
                    }
                    update('table','form','where',where('id',$info['id']),'classhash',$_POST['classhash']);
                    $info['classhash']=$_POST['classhash'];
                }
