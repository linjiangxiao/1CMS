        ...
        if($column=C('cms:form:get',@$_POST['id'])){
            ...
            if($column_edit_array['inputhash']==$column['inputhash']) {
                ...
                foreach($column_config as $val) {
                    ...
                    $val['auth']['all']=true;
                    $column_edit_array['configs'][$val['name']]=C('cms:input:post',$val);
                    if($column_edit_array['configs'][$val['name']]===false) {
                        Return E('配置项:'.$val['configname'].' 不正确');
                    }elseif(is_array($column_edit_array['configs'][$val['name']])) {
                        if(isset($column_edit_array['configs'][$val['name']]['error'])) {
                            Return E('配置项:'.$val['configname'].' '.$column_edit_array['configs'][$val['name']]['error']);
                        }else {
                            Return E('配置项:'.$val['configname'].' 不正确');
                        }
                    }
