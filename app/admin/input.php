        ...
        switch($action) {
            ...
            case 'config':
                Return array(
                            ...
                            array('configname'=>'目录','hash'=>'filepath','inputhash'=>'text','tips'=>'文件保存目录,默认: /upload/(Y)(m)(d)/ ,用户目录:/upload/(userid)/,请确保目录有写入权限'),
                            array('configname'=>'文件名','hash'=>'filename','inputhash'=>'text','tips'=>'默认随机文件名:(rand).(ext) , 固定文件名:logo.png , 原始文件名:(filename).(ext) , 用户id文件名:(userid).(ext)'),
                        );
        }
        Return false;
    }
    ...
    function checkbox($action,$config=array()) {
        ...
        switch($action) {
            ...
            case 'form':
                ...
                if($config['style']==2) {$skin=' lay-skin="tag" ';}else {$skin='';}
                foreach($values as $thisvalue) {
