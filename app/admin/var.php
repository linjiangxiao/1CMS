        ...
        if($var=C('cms:form:get',@$_POST['id'])){
            ...
            if($var_edit_array['inputhash']==$var['inputhash']) {
                ...
                foreach($var_config as $val) {
                    ...
                    $val['auth']['all']=true;
                    $var_edit_array['configs'][$val['name']]=C('cms:input:post',$val);
                    if($var_edit_array['configs'][$val['name']]===false) {
                        Return E('配置项:'.$val['configname'].' 不正确');
                    }elseif(is_array($var_edit_array['configs'][$val['name']])) {
                        if(isset($var_edit_array['configs'][$val['name']]['error'])) {
                            Return E('配置项:'.$val['configname'].' '.$var_edit_array['configs'][$val['name']]['error']);
                        }else {
                            Return E('配置项:'.$val['configname'].' 不正确');
                        }
                    }
            ...
            if(C('cms:form:edit',$var_edit_array)) {
                ...
                if($var_edit_array['inputhash']==$var['inputhash'] && $resetdefault && $var_edit_array['defaultvalue']!==false) {
                    ...
                    foreach($all_channel as $this_channel) {
                        C('cms:channel:setVar',$this_channel,$var['hash'],$var_edit_array['defaultvalue']);
                    }
                }
