...
class admin_user {
    ...
    function roleAddPost() {
        ...
        if(C('cms:user:roleGet',$_POST['hash'])){Return E('角色标识已被使用');}
        $role_add_array['hash']=$_POST['hash'];
        if(isset($_POST['classhash']) && is_hash($_POST['classhash'])) {
            $role_add_array['classhash']=$_POST['classhash'];
        }
    ...
    function roleEditPost() {
        if($role=C('cms:user:roleGet',@$_POST['hash'])){
            ...
            if(isset($_POST['classhash']) && is_hash($_POST['classhash'])) {
                $role_edit_array['classhash']=$_POST['classhash'];
            }elseif(isset($_POST['classhash'])){
                $role_edit_array['classhash']='';
            }
    ...
    function info() {
        if($array['user']=C('cms:user:get',@$_GET['id'])){
            ...
            $userinfo_configs=@json_decode($array['user']['configs'],1);
        ...
        if($array['user']=C('cms:user:get',$_POST['hash'])){
            ...
            if(empty($msg) && count($user_edit_query)) {
                ...
                if(isset($_POST['_last_update_time']) && $_POST['_last_update_time']){
                    $user_edit_query['_last_update_time']=$_POST['_last_update_time'];
                }
