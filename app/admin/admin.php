...
class admin {
    ...
    function nologinActionCheck($do){
        ...
        $doc=C('cms:class:getFunctionDoc',$do);
        if($doc && isset($doc['auth']) && $doc['auth']=='-1'){
            Return true;
        }
        Return false;
    }
    function publicActionCheck($do) {
        ...
        $doc=C('cms:class:getFunctionDoc',$do);
        if($doc && isset($doc['auth']) && $doc['auth']=='0'){
            Return true;
        }
        Return false;
    }
    ...
    function adminLoadAuth($function) {
        $admin_class=explode(':',$function);
        if(!is_hash($admin_class[0])) {
            Return false;
        }
        $class_auth=C('cms:class:authList',$admin_class[0],$function);
        if(is_array($class_auth)) {
            ...
            foreach($auths as $auth) {
                if(stripos($auth,'|false')===false) {
                    if($admin_class[0].':'.$auth===$function) {
                        Return true;
                    }
                }else {
                    if($admin_class[0].':'.$auth===$function) {
    ...
    function formAuth($formid) {
        ...
        $formkinds=array('column'=>'admin:column:index','var'=>'admin:var:index','info'=>'admin:info:index','config'=>'admin:class:permission');
    ...
    function head($title='') {
        ...
        $headhtml.='<script>layui.config({base: \''.template_url().'static/\',version:\'5.0\'}).extend({index: \'lib/index\'}).use([\'index\',\'form\'],function(){});</script>'."\n";
        Return $headhtml;
    }
    function css($check=1) {
        if($check && G('css')){return '';}
        G('css',1);
        Return '<link rel="stylesheet" href="'.template_url().'static/admin.css?v=5.0" media="all">'."\n";
    }
    function icoNav() {
    ...
    function login() {
        ...
        if(isset($_GET['token']) && $userid=C('cms:user:checkToken',$_GET['token']) && C('this:adminCookie',$_GET['token'])){
            if(isset($_GET['home'])){
                $url=C('cms:common:url');
                $url=str_replace("&token=".$_GET['token'],"",$url);
                $url=str_replace("?token=".$_GET['token'].'&',"?",$url);
                $url=str_replace("?token=".$_GET['token'],"?",$url);
                return C('cms:common:jump',$url);
            }
            return C('cms:common:jump','./');
        }
        if(C('this:nowUser')){return C('cms:common:jump','./');}
    ...
    function loginCopyright() {
        ...
        Return ('<div class="layui-trans layadmin-user-login-footer"><p>© '.date('Y').' <a href="https://1cms.com" target="_blank">1CMS.com</a></p></div>');
    }
    function logout() {
        if(!C('this:csrfCheck',1)) {
            if(C('cms:common:isAjax')) {
    ...
    function nowUser() {
        ...
        if(isset($_SERVER['HTTP_AUTHORIZATION']) && strtolower(substr($_SERVER['HTTP_AUTHORIZATION'],0,7))=='bearer '){
            $token=substr($_SERVER['HTTP_AUTHORIZATION'],7);
        }elseif(isset($_COOKIE['token'.C('this:cookieHash')])){
            $token=$_COOKIE['token'.C('this:cookieHash')];
        }else{
            Return false;
        }
        if(!$userid=C('cms:user:checkToken',$token)) {
            Return false;
        }
