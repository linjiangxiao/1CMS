    ...
    function editor($action,$config=array()) {
        switch($action) {
            ...
            case "ajax":
                ...
                if($file_upload=C('cms:common:upload','imgFile',$config['filepath'],$config['filename'])) {
            ...
            case "config":
                Return array(
                            ...
                            array('configname'=>'文件名','hash'=>'filename','inputhash'=>'text','tips'=>'上传文件名格式,默认随机文件名:(rand).(ext) , 原始文件名:(filename).(ext) '),
                        );
        }
        Return false;
    }
