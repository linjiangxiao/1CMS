...
class cms_channel {
    ...
    function get($cid=0,$classhash='') {
        ...
        $configs=@json_decode($channel['configs'],1);
        if(!$configs){ $configs=array(); }
        $vars=C('this:form:all','var',$channel['modulehash'],$channel['classhash']);
        foreach($vars as $var) {
            if(isset($configs[$var['hash']])){
                $channel[$var['hash']]=$configs[$var['hash']];
            }else{
    ...
    function add($channel_add_query) {
        ...
        if(isset($channel_add_query['var'])){
            $channel_add_query['configs']=$channel_add_query['var'];
            unset($channel_add_query['var']);
        }
        if(isset($channel_add_query['configs']) && is_array($channel_add_query['configs'])){
            $channel_add_query['configs']=json_encode($channel_add_query['configs']);
        }
    ...
    function edit($channel_edit_query) {
        ...
        }elseif(isset($channel_edit_query['fid'])) {
        ...
        if(isset($channel_edit_query['configs']) && is_array($channel_edit_query['configs'])){
            $channel_edit_query['configs']=json_encode($channel_edit_query['configs']);
        }
    ...
    function getVar($channel,$varhash) {
        ...
        $configs=@json_decode($channel['configs'],1);
        if(isset($configs[$varhash])){
            return $configs[$varhash];
        }
    ...
    function setVar($channel,$varhash,$value='') {
        ...
        $configs=@json_decode($channel['configs'],1);
        if(!$configs){ $configs=array(); }
        $configs[$varhash]=$value;
        if(!C('this:channel:edit',array('id'=>$channel['id'],'configs'=>$configs))){
            Return false;
        }
        if(isset($GLOBALS['channel'][$channel['id']])) {
            $GLOBALS['channel'][$channel['id']][$varhash]=$value;
            $GLOBALS['channel'][$channel['id']]['configs']=json_encode($configs);
        }
        Return true;
    }
