...
class cms {
    ...
    function notFound() {
        ...
        echo('<html><head><title>404 Not Found</title></head><body><center><h1>404 Not Found</h1></center><hr><center><a href="http://1cms.com" style="color:#000;text-decoration:none" target="_blank">1CMS</a></center></body></html>');
        Return true;
    }
    ...
    function upgrade($old_version) {
        ...
        if(version_compare($old_version,'4.9','<')) {
            C($GLOBALS['C']['DbClass'].':addField','form','configs','longtext');
            C($GLOBALS['C']['DbClass'].':addField','channel','configs','longtext');
            C($GLOBALS['C']['DbClass'].':addField','class','configs','longtext');
            C($GLOBALS['C']['DbClass'].':addField','module','configs','longtext');
            C($GLOBALS['C']['DbClass'].':addField','user','configs','longtext');
            C($GLOBALS['C']['DbClass'].':addField','role','classhash','varchar(32)');
            $configIds=array();
            $forms=array();
            $configs=all('table','config','where',where('hash%',':_form:'));
            foreach ($configs as $config) {
                $keys=explode(':',$config['hash']);
                if(count($keys)==6){
                    $forms[$keys[0].':'.$keys[2].':'.$keys[3].':'.$keys[4]][$keys[5]]=$config['value'];
                    $configIds[]=$config['id'];
                }
            }
            foreach ($forms as $key => $form) {
                $keys=explode(':',$key);
                update('table','form','where',where('classhash',$keys[0],'modulehash',$keys[1],'kind',$keys[2],'hash',$keys[3]),'configs',json_encode($form));
            }
            $channels=array();
            $configs=all('table','config','where',where('hash%',':article:var:'));
            foreach ($configs as $config) {
                $keys=explode(':',$config['hash']);
                if(count($keys)==5){
                    $channels[$keys[1]][$keys[4]]=$config['value'];
                    $configIds[]=$config['id'];
                }
            }
            foreach ($channels as $key => $channel) {
                update('table','channel','where',where('id',$key),'configs',json_encode($channel));
            }
            if($configIds){
                del('table','config','where',where('id',$configIds));
            }
        }
    }
}
function CMS_init() {
    if(!defined('1cms')){define('1cms',1);}
    if(!isset($GLOBALS['C']['UrlRewrite'])) {$GLOBALS['C']['UrlRewrite']=1;}
    if(!isset($GLOBALS['C']['SiteHash'])) {$GLOBALS['C']['SiteHash']=md5(dirname(__FILE__));}
    if(!isset($GLOBALS['C']['ClassDir'])) {$GLOBALS['C']['ClassDir']='app';}
    ...
    if(isset($GLOBALS['C']['DbInfo']) && is_array($GLOBALS['C']['DbInfo'])) {
        ...
        $GLOBALS['route'][]=array('uri'=>'/(1cms)','classhash'=>'cms','classfunction'=>'install:goInstall');
        $GLOBALS['route'][]=array('uri'=>'/(1cms)/','classhash'=>'cms','classfunction'=>'install:goInstall');
    }
...
class cms_database {
    ...
    function delField($table,$name){
        ...
        if($this->kind=='sqlitepdo') {
            ...
            $this->query("INSERT INTO ".$this->prefix($tablenew)." SELECT $fieldnamessql FROM ".$this->prefix($table).";");
        ...
        $this->query("alter table ".$this->prefix($table)." drop $name;");
        Return true;
    }
