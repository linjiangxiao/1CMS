...
class cms_common {
    ...
    function url(){
        if(C('this:common:serverHttps')){
            return 'https://'.C('cms:common:serverName').C('cms:common:serverPort').$_SERVER['REQUEST_URI'];
        }
    ...
    function markdown($code,$safemode=0) {
        if (version_compare(PHP_VERSION, '5.3', '<')) {
            return 'markdown error';
        }
        return C('this:markdown:text',$code,$safemode);
    }
    ...
    function upload($name,$path='',$filename='') {
        ...
        foreach($allfiles as $filekey=>$file) {
            ...
            $replace_value=array($file_ext,substr(md5(time().rand(1000000, 9999999).@$file['tmp_name']),0,14),implode('.',$temp_arr),date('Y'),date('y'),date('m'),date('d'),date('H'),date('h'),date('i'),date('s'));
            if(strpos($path,'(userid)')!==false || strpos($filename,'(userid)')!==false){
                $replace_key[]='(userid)';
                if($userid=C('admin:nowUser')){
                    $replace_value[]=$userid;
                }else{
                    $replace_value[]=0;
                }
            }
            if(strpos($path,'(hash')!==false || strpos($filename,'(hash')!==false){
                $md5=@md5_file($file['tmp_name']);
                if(!$md5){
                    $md5=md5(time().rand(1000000, 9999999).@$file['tmp_name']);
                }
                $replace_key[]='(hash)';
                $replace_value[]=$md5;
                for ($i=1; $i <=6 ; $i++) {
                    $replace_key[]='(hash'.$i.')';
                    $replace_value[]=substr($md5,0,$i);
                }
            }
    ...
    function uploadExt() {
        Return array('gif','jpg','jpeg','png','bmp','ico','blob','psd','svg','webp','doc','docx','xls','xlsx','ppt','txt','zip','7z','gz','bz2','pdf','rar','tar','torrent','exe','apk','ipa','swf','flv','mp3','mp4','wav','wma','wmv','mid','avi','mpg','asf');
    }
