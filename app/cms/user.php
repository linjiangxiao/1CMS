...
class cms_user {
    ...
    function edit($user_edit_query) {
        ...
        if(isset($user_edit_query['_last_update_time'])){
            $userinfo_configs=@json_decode($userinfo['configs'],1);
            if(isset($userinfo_configs['last_update_time']) && $user_edit_query['_last_update_time']!=$userinfo_configs['last_update_time']){
                return E('数据不一致');
            }
            unset($user_edit_query['_last_update_time']);
        }
        if(!isset($user_edit_query['configs'])){
            $user_edit_query['configs']=@json_decode($userinfo['configs'],1);
            if(!$user_edit_query['configs']){
                $user_edit_query['configs']=array();
            }
        }
        if(isset($user_edit_query['configs']) && is_string($user_edit_query['configs'])){
            $user_edit_query['configs']=@json_decode($user_edit_query['configs'],1);
        }
        if(isset($user_edit_query['configs']) && is_array($user_edit_query['configs'])){
            $user_edit_query['configs']['last_update_time']=time();
            $user_edit_query['configs']=@json_encode($user_edit_query['configs']);
        }
    ...
    function roleAdd($role_add_query) {
        ...
        if(isset($role_add_query['classhash']) && !is_hash($role_add_query['classhash'])) {
            $role_add_query['classhash']='';
        }
        if(!isset($role_add_query['classhash'])){
            $role_add_query['classhash']=I(-1);
        }
