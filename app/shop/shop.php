...
class shop {
    ...
    function index() {
        ...
        if($array['content']===false) {$array['content']='<meta http-equiv=refresh content=\'5; url=\'><i class="layui-icon layui-icon-loading layui-icon layui-anim layui-anim-rotate layui-anim-loop"></i>重新连接中';}
    ...
    function shopInfo($data=array()) {
        ...
        $data['_domain']=@server_name();$data['_hash']=@$GLOBALS['C']['SiteHash'];$data['_ip']=C('cms:common:ip');$data['_uid']=C('admin:nowuser');$data['_referer']=@$_SERVER['HTTP_REFERER'];$data['_ua']=@$_SERVER['HTTP_USER_AGENT'];$data['_php']=@PHP_VERSION;$data['_os']=@php_uname('s');$data['_time']=time();if(isset($GLOBALS['C']['aff'])){$data['_aff']=$GLOBALS['C']['aff'];}
    ...
    function defaultHost() {
        Return '1cms.com;1cms.classcms.com';
    }
    function shopHost() {
    ...
    function get() {
        $last_connect_time=config('last_connect_time');
            if(time()-$last_connect_time<60){
            if(!$connect_count=config('connect_count')){ $connect_count=0; }
            if($connect_count>30){
                if(isset($_GET['ajax']) && $_GET['ajax']) {
                    Return false;
                }
                Return '访问频繁,请稍后再试';
            }elseif(time()-$last_connect_time>5){
                config('connect_count',$connect_count-1);
            }else{
                config('connect_count',$connect_count+1);
            }
        }else{
            config('connect_count',0);
        }
        config('last_connect_time',time());
        $host=C('this:shopHost');
        $url='http://'.$host.'/shopapi/';
        if (function_exists("curl_init")){
